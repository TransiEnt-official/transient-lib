...
//________________________________________________________________________________//
  ...
  TransiEnt.Components.Gas.VolumesValvesFittings.Pipes.PipeFlow_L4_Simple pipe_54(
    ...
    redeclare model PressureLoss = ClaRa.Basics.ControlVolumes.Fundamentals.PressureLoss.Generic_PL.LinearPressureLoss_L4)
                                                                                                                   annotation (Placement(transformation(extent={{-70,-5},{-42,5}})));
  ...
  TransiEnt.Components.Gas.VolumesValvesFittings.Pipes.PipeFlow_L4_Simple pipe_32(
    ...
    N_cv=2,
    redeclare model PressureLoss = ClaRa.Basics.ControlVolumes.Fundamentals.PressureLoss.Generic_PL.LinearPressureLoss_L4)
                                                                                                                   annotation (Placement(transformation(extent={{42,-5},{70,5}})));
                                                                      //length like given in the paper, d_i scaled with 1/10
...
equation
  ...
  connect(temp3.gasPortOut, pipe_32.gasPortIn) annotation (Line(
      ...
      color={255,255,0},
      thickness=1.5));
  connect(vehicle_tank.gasPortOut, boundary_Txim_flow1.gasPort) annotation (Line(
      points={{92.3,-3.88578e-16},{100,0}},
      color={255,255,0},
      thickness=1.5));
  connect(boundary_Txim_flow.gasPort, station_tank.gasPortIn) annotation (Line(
      points={{-100,0},{-90.9,8.88178e-16}},
      color={255,255,0},
      thickness=1.5));
  ...
  connect(ht_pipe_54.heat, wall_pipe_54.outerPhase) annotation (Line(
      ...
      color={167,25,48},
      thickness=0.5));
  connect(ht_pipe_32.heat, wall_pipe_32.outerPhase) annotation (Line(
      points={{56,40},{56,28}},
      color={167,25,48},
      thickness=0.5));
  connect(T_amb.y, ht_pipe_32.T_outer) annotation (Line(points={{-1.9984e-15,75},{0,75},{0,70},{56,70},{56,60}}, color={0,0,127}));
  connect(T_amb.y, ht_pipe_54.T_outer) annotation (Line(points={{0,75},{0,70},{-56,70},{-56,60}}, color={0,0,127}));
