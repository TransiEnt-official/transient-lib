...
public
  ...
  parameter SI.Voltage v_n=exponentialStaticConsumerModel.simCenter.v_n
    "Nominal voltage";
  parameter SI.Angle delta_load_start=-0.08726646259971647;
  parameter SI.ActivePower P_n=50
  ...
  TransiEnt.Consumer.Electrical.Base.ExponentialStatic
    exponentialStaticConsumerModel(
    data=variability,
    f_n=f_n,
    v_n=v_n) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={-33,48})));
  TransiEnt.Basics.Interfaces.Electrical.ComplexPowerPort epp annotation (
      Placement(transformation(extent={{-106,-10},{-86,10}}),
        iconTransformation(extent={{-108,-10},{-88,10}})));
  TransiEnt.Components.Boundaries.Electrical.ComplexPower.PQBoundary boundary(
      v_n=v_n, delta_cp(start=delta_load_start)) annotation (choicesAllMatching=
       true, Placement(transformation(extent={{42,-64},{62,-44}})));
  TransiEnt.Components.Statistics.Collectors.LocalCollectors.CollectElectricPower
    collectElectricPower(typeOfResource=TransiEnt.Basics.Types.TypeOfResource.Consumer,
      integrateElPower=integrateElPower)
  ...
  Modelica.Blocks.Sources.RealExpression P_set(y=P_internal) annotation (
      Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={24,4})));
  Modelica.Blocks.Sources.RealExpression Q_set(y=if not useCosPhi then Q_el_set
         else abs(P_internal/cosphi_set)*sin(behavior*acos(cosphi_set)))
  ...
  Modelica.Blocks.Sources.RealExpression f_grid(y=epp.f) annotation (Placement(
        transformation(extent={{-92,44},{-72,64}}, rotation=0)));
  Modelica.Blocks.Math.Product ActivePower annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=0,
        origin={2,36})));
  Modelica.Blocks.Sources.RealExpression P_el_n(y=P_n) annotation (Placement(
        transformation(extent={{-66,-26},{-46,-6}}, rotation=0)));
  Modelica.Blocks.Math.Product ReactivePower annotation (Placement(
        transformation(
        extent={{-6,-6},{6,6}},
        rotation=0,
        origin={2,64})));
  Modelica.Blocks.Sources.RealExpression Q_el_n(y=Q_n) annotation (Placement(
        transformation(extent={{-60,66},{-40,86}}, rotation=0)));
...
equation
  ...
  connect(epp, electricVoltageComplex.epp) annotation (Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(electricVoltageComplex.v, exponentialStaticConsumerModel.v)
    annotation (Line(points={{-56,32},{-50,32},{-50,43.2},{-42.24,43.2}}, color=
         {0,0,127}));
  connect(exponentialStaticConsumerModel.f, f_grid.y)
    annotation (Line(points={{-42.24,54},{-71,54}}, color={0,0,127}));
  connect(exponentialStaticConsumerModel.Delta_P_star, ActivePower.u1)
    annotation (Line(points={{-21.78,42.8},{-15.89,42.8},{-15.89,39.6},{-5.2,39.6}},
        color={0,0,127}));
  connect(P_el_n.y, ActivePower.u2) annotation (Line(points={{-45,-16},{-14,-16},
          {-14,32.4},{-5.2,32.4}}, color={0,0,127}));
  connect(exponentialStaticConsumerModel.Delta_Q_star, ReactivePower.u2)
    annotation (Line(points={{-21.78,54},{-12,54},{-12,60.4},{-5.2,60.4}},
        color={0,0,127}));
  connect(ReactivePower.u1, Q_el_n.y) annotation (Line(points={{-5.2,67.6},{-12,
          67.6},{-12,76},{-39,76}}, color={0,0,127}));
  connect(P_set.y, ActiveSum.u2) annotation (Line(points={{35,4},{44,4},{44,-2.8},
          {44.4,-2.8}}, color={0,0,127}));
  connect(ActivePower.y, ActiveSum.u1) annotation (Line(points={{8.6,36},{51.3,36},
          {51.3,-2.8},{51.6,-2.8}}, color={0,0,127}));
  connect(Q_set.y, ReactiveSum.u1) annotation (Line(points={{75,8},{72,8},{72,-2.8},
          {71.6,-2.8}}, color={0,0,127}));
  connect(ReactiveSum.u2, ReactivePower.y) annotation (Line(points={{64.4,-2.8},
          {64.4,64.6},{8.6,64.6},{8.6,64}}, color={0,0,127}));
  connect(ActiveSum.y, boundary.P_el_set)
    annotation (Line(points={{48,-16.6},{48,-42},{46,-42}}, color={0,0,127}));
  connect(ReactiveSum.y, boundary.Q_el_set) annotation (Line(points={{68,-16.6},
          {64,-16.6},{64,-42},{58,-42}}, color={0,0,127}));
  annotation (
