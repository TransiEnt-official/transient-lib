...
//________________________________________________________________________________//
  ...
  //Basics
  parameter TILMedia.VLEFluidTypes.BaseVLEFluid medium=simCenter.fluid1 annotation (Dialog(               group="General", enable=useFluidPorts));
  parameter Boolean useHomotopy=simCenter.useHomotopy "true =  homotopy method is used during initialisation" annotation (Dialog(               group="General", enable=useFluidPorts));
  ...
  parameter Boolean noFriction=true "true = assume no pressure loss due to friction" annotation(Dialog(group="Pressure drop", enable=useFluidPorts));
  ...
  parameter Real b(unit="1/(kg.m)") = 0 "Quadratic pressure drop coefficient"  annotation (Dialog(group="Pressure drop", enable=useFluidPorts));
  parameter SI.Height z1=0 "Height inlet" annotation (Dialog(group="Pressure drop", enable=useFluidPorts));
  parameter SI.Height z2=0 "Height outlet" annotation (Dialog(group="Pressure drop", enable=useFluidPorts));
...
//protected
  Modelica.Units.SI.SpecificEnthalpy h_out "Enthalpy of the medium flowing out of the collector";
  ...
  // _____________________________________________
public
  TransiEnt.Basics.Interfaces.Thermal.FluidPortOut waterPortOut(Medium=medium) if useFluidPorts annotation (Placement(transformation(extent={{70,-10},{90,10}})));
  TransiEnt.Basics.Interfaces.Thermal.FluidPortIn waterPortIn(Medium=medium) if useFluidPorts annotation (Placement(transformation(extent={{-90,-10},{-70,10}})));
  ...
  TransiEnt.Basics.Interfaces.General.TemperatureOut T_out annotation (Placement(transformation(extent={{-10,-10},{10,10}},
        rotation=90,
        origin={54,90}), iconTransformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={60,90})));
  ...
  Modelica.Blocks.Interfaces.RealOutput Q_flow_out annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={102,54}),iconTransformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={102,54})));
...
public
  ...
  TransiEnt.Components.Statistics.Collectors.LocalCollectors.CollectGwpEmissionsHeat collectGwpEmissions(typeOfEnergyCarrierHeat=TransiEnt.Basics.Types.TypeOfPrimaryEnergyCarrierHeat.Solar) annotation (Placement(transformation(extent={{-20,100},{0,80}})));
        ...
        extent={{11,-11},{-11,11}},
        rotation=90,
        origin={-93,91}),  iconTransformation(
        extent={{11,-11},{-11,11}},
        rotation=90,
        origin={-93,91})));
  ...
  Basics.Interfaces.General.TemperatureIn T_inflow if not useFluidPorts annotation (Placement(transformation(
        extent={{11,-11},{-11,11}},
        rotation=90,
        origin={-75,91}),  iconTransformation(
        extent={{11,-11},{-11,11}},
        rotation=90,
        origin={-69,101})));
  Basics.Interfaces.General.TemperatureOut T_in if useFluidPorts annotation (Placement(transformation(
        extent={{-11,-11},{11,11}},
        rotation=90,
...
if not useFluidPorts then
  ...
  Q_flow_collector=m_flow_int*(h_out-h_in);

  if abs(Modelica.Units.Conversions.to_deg(irradiance.angle_direct_tilted)) <= 90 then
 ...
 if use_input_data then
  ...
  connect(heatFlow2fluid.waterPortOut, waterPortOut) annotation (Line(
      points={{6.6,6.2},{64,6.2},{64,0},{80,0}},
      color={175,0,0},
      thickness=0.5));
  connect(heatFlow2fluid.Q_flow_out, Q_flow_out) annotation (Line(points={{8.2,11.4},{66,11.4},{66,54},{102,54}}, color={0,0,127}));
  connect(m_flow, m_flow_int) annotation (Line(points={{-93,91},{-93,51}},                    color={0,0,127}));
  connect(waterPortIn, massFlowSensorVLE.inlet) annotation (Line(
      points={{-80,0},{-56,0},{-56,2},{-50,2}},
      color={175,0,0},
      thickness=0.5));
  connect(heatFlow2fluid.waterPortIn, massFlowSensorVLE.outlet) annotation (Line(
      points={{-10.4,6.2},{-26,6.2},{-26,2},{-30,2}},
      color={175,0,0},
      thickness=0.5));
...
<p>A simple solar collector providing useful energy gain as recommended by EN 12975 steady state thermal performance equation [1]. Effective heat capacity was added for transient behavior according to [2].</p>
...
<p>Model modified by Anne Hagemeier (anne.hagemeier@umsicht.fraunhofer.de), Aug 2021: enabled the model to be useable without fluid ports</p>
