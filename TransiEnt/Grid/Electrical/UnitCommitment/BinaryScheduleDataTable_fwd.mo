...
//________________________________________________________________________________//
  ...
  Modelica.Blocks.Interfaces.BooleanOutput[nDispPlants,ntime] z annotation (Placement(transformation(extent={{93,21},{133,59}})));
  ...
  TransiEnt.Basics.Blocks.DelayedBooleanReplicator delayedBooleanReplicator(
    samplePeriod=samplePeriod,
    nout=nDispPlants,
    t_pred=-t_prediction) annotation (Placement(transformation(extent={{54,30},{74,50}})));
equation
  // _____________________________________________
  //
  ...
  for i in 1:nDispPlants loop
    delayedBooleanReplicator.u[i] = (not unit_blocked[i] and -prediction.y[i] >= P_min[i]) or unit_mustrun[i];
    P_sec_pos[i] = reserveAllocation.y[nPlants+controlPlants[i]];
    P_sec_neg[i] = reserveAllocation.y[2*nPlants+controlPlants[i]];
  end for;

  connect(delayedBooleanReplicator.y, z) annotation (Line(points={{75,40},{113,40}}, color={255,0,255}));
...
<p>z[nDispPlants, ntime]: BooleanOutput</p>
...
<p>Tested in check model &quot;TransiEnt.Grid.Electrical.UnitCommitment.Check.TestBinarySchedule_fwd&quot;</p>
<h4><span style=\"color: #008000\">9. References</span></h4>
<p>(no remarks)</p>
<h4><span style=\"color: #008000\">10. Version History</span></h4>
</html>"));
end BinaryScheduleDataTable_fwd;
