...
//________________________________________________________________________________//
  ...
  parameter Real Soiling=5
    "Average annual losses of radiation in % due to soiling";
  parameter Real Albedo=0.25
    "Average annual losses of radiation in % due to soiling";
  parameter String DiffuseModel="Skartveit and Olseth"
    "Choose the diffuse fraction correlation" annotation (choices(
      choice="Skartveit and Olseth",
      choice="Erbs",
      choice="Orgill and Hollands",
      choice="Reindl et al. 1",
      choice="Reindl et al. 2"));
  ...
  parameter Integer kind(
    min=1,
    max=4) = 1 "IAM for direct Irradiance" annotation (Dialog(tab="IAM", group="General"),
      choices(
  ...
  parameter Real constant_iam_dir=1 "constant IAM for direct irradiation"
    annotation (Dialog(tab="IAM", group="General"));
  ...
  replaceable model Skymodel =
      ...
      totaldays=totaldays) constrainedby TransiEnt.Producer.Heat.SolarThermal.Base.SkymodelBase
                                                           "choose sky model"
    annotation (choicesAllMatching=true, Dialog(tab="Irradiance", group="Skymodel"));
  parameter Real reflectance_ground=0.2 "reflectance of the ground"
    annotation (Dialog(tab="Irradiance", group="Skymodel"));
  parameter Boolean direct_normal=true
    "Is the direct irradiance measured on a surface normal to irradiance?"
    annotation (Dialog(tab="Irradiance", group="Skymodel"));
  parameter Boolean integratePowerDc=false
    "True if the power shall be integrated";
  parameter Boolean integratePowerOut=false
    "True if the power output shall be integrated";
  // _____________________________________________
  //
  //                    Variables
  // _____________________________________________
...
equation
  ...
  connect(gHI_Splitter.irradiance_diffuse_horizontal, irradiance.irradiance_diffuse_horizontal_input)
    annotation (Line(points={{-69,-4},{-66,-4},{-66,-5.6},{-61,-5.6}}, color={0,
          0,127}));
  connect(gHI_Splitter.irradiance_direct_horizontal, irradiance.irradiance_direct_measured_input)
    annotation (Line(points={{-69,4},{-66,4},{-66,5.6},{-61,5.6}}, color={0,0,127}));
  annotation (
