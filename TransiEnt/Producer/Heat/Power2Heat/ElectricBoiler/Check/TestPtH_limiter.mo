...
//________________________________________________________________________________//
  ...
  Modelica.Blocks.Sources.RealExpression Temperature(y=45*4200)     annotation (Placement(transformation(extent={{-94,-43},{-68,-29}})));
  ...
  TransiEnt.Components.Boundaries.FluidFlow.FluidSource fluidSource annotation (Placement(transformation(extent={{-52,-44},{-32,-24}})));
  TransiEnt.Components.Boundaries.FluidFlow.FluidSink fluidSink annotation (Placement(transformation(extent={{78,-6},{58,14}})));
  Modelica.Blocks.Sources.RealExpression realExpression2(y=1e5) annotation (Placement(transformation(extent={{110,-6},{90,14}})));
equation
  ...
  connect(ptH_limiter.Q_flow_set_demand, heatDemand.y) annotation (Line(
      ...
      color={162,29,33},
      pattern=LinePattern.Dash));
  connect(PtH.Q_flow_set, gain.y) annotation (Line(points={{-13.4,-33},{-13.4,-16},{0.5,-16},{0.5,-10.35}},
                                                                                                      color={0,0,127}));
  connect(ElectricGrid.epp, PtH.epp) annotation (Line(
      points={{2,-60},{-3,-60},{-3,-44.2}},
      color={0,135,135},
      thickness=0.5));
  connect(realExpression2.y, fluidSink.p_in) annotation (Line(points={{89,4},{76,4}}, color={0,0,127}));
  connect(fluidSink.port_a, PtH.fluidPortOut) annotation (Line(points={{58,4},{14,4},{14,-34},{7.2,-34}}, color={0,0,0}));
  connect(fluidSource.port_a, PtH.fluidPortIn) annotation (Line(points={{-32,-34},{-13.4,-34}}, color={0,0,0}));
  connect(massFlow.y, fluidSource.m_flow_in) annotation (Line(points={{-66.7,-24},{-56,-24},{-56,-31},{-50,-31}}, color={0,0,127}));
  connect(Temperature.y, fluidSource.h_in) annotation (Line(points={{-66.7,-36},{-50,-36}}, color={0,0,127}));
