...
model SolarThermalSystem_3LayerStorage "Energy based combination of solar collector, controller, boiler and thermal storage with three layers"
...
//________________________________________________________________________________//
  ...
  parameter Real p_Volume[storage.N_cv]={0.2,0.3,0.5} "Proportion of the total volume for the three parts of the tank" annotation (Dialog(group="Storage"));
  ...
  Modelica.Blocks.Sources.RealExpression T_in(y=storage.port[3].T) annotation (Placement(transformation(
  ...
  Modelica.Blocks.Sources.RealExpression Q_flow_heating2(y=max(-HeatingSolar, -m_flow_consumer*4185*(storage.port[2].T - T_return))) annotation (Placement(transformation(extent={{30,-2},{14,10}})));
  ...
  Modelica.Blocks.Sources.RealExpression Q_flow_DHW3(y=max(-Q_flow_demand_hotwater, min(0, -m_flow_hotwater*4185*(storage.port[3].T - 12 - 273.15)))) annotation (Placement(transformation(extent={{30,-34},{14,-22}})));
  Modelica.Blocks.Sources.RealExpression Q_flow_DHW2(y=max(-Q_flow_demand_hotwater - Q_flow_DHW3.y, min(0, -m_flow_hotwater*4185*(storage.port[2].T - storage.port[3].T)))) annotation (Placement(transformation(extent={{30,-12},{14,0}})));
  Modelica.Blocks.Sources.RealExpression Q_flow_DHW1(y=-(Q_flow_demand_hotwater + Q_flow_DHW2.y + Q_flow_DHW3.y)) annotation (Placement(transformation(extent={{30,20},{14,32}})));
  ...
  TransiEnt.Storage.Heat.HotWaterStorage_constProp_L4.HotWaterStorage_constProp_L4 storage(
    ...
    k=k,
    rho=rho,
    c_v=c_v) annotation (Placement(transformation(extent={{-82,-14},{-60,8}})));
  ...
  TransiEnt.Producer.Heat.SolarThermal.SolarCollector_L1_constProp solarCollector(
    ...
    direct_normal=direct_normal) annotation (Placement(transformation(extent={{62,-24},{80,-6}})));

  TransiEnt.Producer.Heat.SolarThermal.Control.ControllerPumpSolarCollectorTandG controller(
    T_set=T_set,
    G_min=G_min,
    T_stor_max=T_max,
    P_drive_min(k=controller.m_flow_min)) annotation (Placement(transformation(extent={{74,34},{100,54}})));
...
equation
 ...
 // _____________________________________________
  ...
  connect(T_in.y, controller.T_in) annotation (Line(points={{61.65,16.5},{64,16.5},{64,46},{75.7333,46}}, color={0,0,127}));
  connect(T_in.y, controller.T_stor) annotation (Line(points={{61.65,16.5},{64,16.5},{64,34},{75.7333,34}}, color={0,0,127}));
  ...
  connect(heatFlowRate_boiler.y, boiler.Q_flow_set) annotation (Line(points={{-37,-50},{-28,-50},{-28,-70},{-18,-70}}, color={0,0,127}));
  connect(gasPortIn, boiler.gasIn) annotation (Line(
      points={{76,-98},{-16,-98},{-16,-90},{-17.8,-90}},
      color={255,255,0},
      thickness=1.5));
  connect(controller.P_drive, solarCollector.m_flow) annotation (Line(
      points={{75.7333,49},{42,49},{42,-6},{56,-6},{56,-6.81},{62.63,-6.81}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(T_in.y, solarCollector.T_inflow) annotation (Line(points={{61.65,16.5},{66,16.5},{66,0},{64.79,0},{64.79,-5.91}}, color={0,0,127}));
...
end SolarThermalSystem_3LayerStorage;
