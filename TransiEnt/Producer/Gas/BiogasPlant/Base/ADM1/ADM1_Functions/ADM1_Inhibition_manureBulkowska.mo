...
//________________________________________________________________________________//
  ...
  replaceable Records.ADM1_parameters_manure_bulkowska parameters annotation (Placement(transformation(extent={{-8,-8},{12,12}})));
...
algorithm
  ...
  I_pH := if pH < parameters.pH_UL then exp(-3*((pH - parameters.pH_UL)/(parameters.pH_UL - parameters.pH_LL))^2) else 1 "lower pH-Inhibition if pH Value below Uper Level (pH_UL) affecting most microorganims";
  I_pH_ac := if pH < parameters.pH_UL_ac then exp(-3*((pH - parameters.pH_UL_ac)/(parameters.pH_UL_ac - parameters.pH_LL_ac))^2) else 1 "lower pH-Inhibition if pH Value below Uper Level (pH_UL) affecting Acetate degraders";
  I_pH_h2 := if pH < parameters.pH_UL_h2 then exp(-3*((pH - parameters.pH_UL_h2)/(parameters.pH_UL_h2 - parameters.pH_LL_h2))^2) else 1 "lower pH-Inhibition if pH Value below Uper Level (pH_UL) affecting Hydrogen degraders";
...
<p>This function calculates the inhibition coefficients for the adm1 model using constants from [1,2]. It considers inhibtion due to an unfavorable pH-value, an excessive concentration of hydrogen or ammonia and an insufficient nitrogen concentration.</p>
<h4><span style=\"color: #008000\">Refences</span></h4>
<p><span style=\"font-family: Arial;\">[1] Bułkowska, K., et al. (2015). ADM1-based modeling of anaerobic codigestion of maize silage and cattle manure&ndash;calibration of parameters and model verification (part II)</span></p>
<p><span style=\"font-family: Arial;\">[2] Bułkowska, K., et al. (2015).ADM1-based modeling of anaerobic codigestion of maize silage and cattle manure &ndash; a feedstock characterisation for model implementation (part I)</span></p>
