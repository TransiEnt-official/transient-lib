...
//________________________________________________________________________________//  extends TransiEnt.Basics.Icons.Checkmodel;
  ...
  //   Components
  // ------------------------------------------------------------------------------------------
  Components.Boundaries.Electrical.ComplexPower.SlackBoundary     electricGrid_1(v_gen=10e3, f_n=50)
                          annotation (Placement(transformation(extent={{32.07,31.9},{47.07,46.9}})));

  // ---- LV Grids -------------------------------------------------------------------------------

  LV_semiurb_4 lV_semiurb_4_3(
    ...
    weatherYear=weatherYear) annotation (Placement(transformation(extent={{8,-14},{28,6}})));
  // ------------------------------------------------------------------------------------------
  //   Interfaces
  // ------------------------------------------------------------------------------------------
...
equation
  connect(lV_semiurb_4_3.epp, electricGrid_1.epp) annotation (Line(
      points={{18,6.8},{18,39.4},{32.07,39.4}},
      color={28,108,200},
      thickness=0.5));
  connect(lV_semiurb_4_3.controlBus, controlBus) annotation();
  annotation (
