...
//________________________________________________________________________________//
  ...
  inner TransiEnt.ModelStatistics modelStatistics annotation (Placement(transformation(extent={{-280,260},{-260,240}})));
  inner TransiEnt.SimCenter simCenter(
    thres=1e-9,
    useThresh=true,
    tableInterpolationSmoothness=Modelica.Blocks.Types.Smoothness.ConstantSegments,
    Td=450,
    redeclare TransiEnt.Examples.Hamburg.ExampleGenerationPark2035 generationPark) annotation (Placement(transformation(extent={{-290,240},{-270,260}})));
  ...
  Modelica.Blocks.Sources.RealExpression P_set_WT1(y=-mod.y[UC.schedule.GUDTS]) annotation (Placement(transformation(extent={{190,-126},{210,-106}})));
  ...
  Modelica.Blocks.Sources.RealExpression P_residual_is(y=Demand.epp.P + P_RE_curtailement.epp_out.P + PumpedStorage.epp.P + Biomass.epp.P + P_tieline_set.y) annotation (Placement(transformation(extent={{-268,62},{-248,82}})));
  Modelica.Blocks.Sources.RealExpression P_residual_pred(y=-sum(UC.schedule.y[simCenter.generationPark.isMOD])) "All disponible plants (Conventional apart from Pumped Storage and CHP)" annotation (Placement(transformation(extent={{-262,84},{-242,104}})));
  ...
  Modelica.Blocks.Sources.RealExpression P_set_PV(y=P_PV_is.y1 + e_pv_prediction.y) annotation (Placement(transformation(extent={{12,212},{32,232}})));
...
equation
  ...
  connect(Tiefstack_HardCoal.Q_flow_set, Q_flow_set_WT.y) annotation (Line(points={{154.19,-146.033},{153.985,-146.033},{153.985,-138},{152,-138},{152,-136.9}}, color={0,0,127}));
  connect(HW_HafenCity.Q_flow_set, Q_flow_set_Hafen.y) annotation (Line(points={{9,-190},{9,-187},{-2.1,-187},{-2.1,-185}}, color={0,0,127}));
  ...
  connect(P_set_SB_BM.y, Biomass.P_SB_set) annotation (Line(points={{79,112},{82,112},{82,106.91},{85.2,106.91}}, color={0,0,127}));
  ...
  connect(H_lpa.y, discretizePrediction.P_is) annotation (Line(points={{-217,72},{-211.5,72},{-204,72}}, color={0,0,127}));
  connect(UC.P_sec_pos[simCenter.generationPark.isMOD], mod.P_R_pos) annotation (Line(points={{-166.7,107.4},{-158.8,107.4},{-158.8,86.6}}, color={0,0,127}));
  connect(UC.P_sec_neg[simCenter.generationPark.isMOD], mod.P_R_neg) annotation (Line(points={{-166.7,102.8},{-149.2,102.8},{-149.2,86.6}}, color={0,0,127}));
  connect(P_residual_pred_1h.y, discretizePrediction.P_prediction) annotation (Line(points={{-203,100},{-192,100},{-192,84}}, color={0,0,127}));
