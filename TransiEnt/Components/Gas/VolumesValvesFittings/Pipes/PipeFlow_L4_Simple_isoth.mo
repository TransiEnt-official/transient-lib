...
//________________________________________________________________________________//
  ...
  parameter ClaRa.Basics.Units.MassFraction xi_start[medium.nc - 1]=xi_nom "Initial composition for single tube" annotation (Dialog(tab="Initialisation"));
  ...
  replaceable model PressureLoss = Base.PhysicalPL_L4                                                          constrainedby ClaRa.Basics.ControlVolumes.Fundamentals.PressureLoss.PressureLossBaseVLE_L4
  ...
  TransiEnt.Components.Gas.VolumesValvesFittings.Pipes.PipeFlow_L4_Simple pipe(
    ...
    redeclare final model HeatTransfer = ClaRa.Basics.ControlVolumes.Fundamentals.HeatTransport.Generic_HT.Constant_L4 (alpha_nom=10e4),
...
equation
  ...
  connect(pipe.gasPortOut, gasPortOut) annotation (Line(
      ...
      color={255,255,0},
      thickness=1.5));
  connect(realExpression.y, ht.T_outer) annotation (Line(points={{-47,28},{-32,28}}, color={0,0,127}));
  connect(ht.heat, pipe.heat) annotation (Line(
      points={{-12,28},{0,28},{0,3.33333}},
      color={167,25,48},
      thickness=0.5));
  annotation (
