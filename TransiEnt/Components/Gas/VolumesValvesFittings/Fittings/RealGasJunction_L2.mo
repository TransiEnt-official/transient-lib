...
public
  ...
  parameter SI.Pressure p_min_assert=0 "Minimum pressure in component below which the simulation terminates" annotation(Dialog(group="Fundamental Definitions"));
  parameter SI.Pressure p_max_assert=1000e5 "Maximum pressure in component above which the simulation terminates" annotation(Dialog(group="Fundamental Definitions"));
...
protected
  ...
  inner TILMedia.Internals.VLEFluidConfigurations.FullyMixtureCompatible.VLEFluid_pT gasBulk(
    ...
    T=heat.T,
    xi=xi,
    stateSelectPreferForInputs=false,
...
public
  ClaRa.Basics.Units.MassFraction xi[medium.nc - 1](start=xi_start,stateSelect={if not constantComposition and Modelica.Math.Vectors.find(j, dependentCompositionEntries) == 0 then StateSelect.always else StateSelect.never for j in 1:medium.nc - 1});
...
equation
  assert(p > p_min_assert,"Pressure in component " + getInstanceName() + " is too low! (below " + String(p_min_assert/1e5) + " bar)",AssertionLevel.error);
  assert(p < p_max_assert,"Pressure in component " + getInstanceName() + " is too high! (above " + String(p_max_assert/1e5) + " bar)",AssertionLevel.error);
