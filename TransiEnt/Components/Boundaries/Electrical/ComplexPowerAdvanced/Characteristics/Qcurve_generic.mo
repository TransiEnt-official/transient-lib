...
//________________________________________________________________________________//
  ...
  parameter Modelica.Units.SI.ReactivePower Q_max_star=0.41;
    parameter Real lowerLimit=0.94 "percent of v_n";
...
equation
  ...
  if voltageIn <= lowerLimit*v_n then
    reactivePowerOut = -Q_max_star;

  elseif voltageIn > lowerLimit*v_n and voltageIn < upperLimit*v_n then

  reactivePowerOut=(2*Q_max_star/((upperLimit - lowerLimit)*v_n))*voltageIn + Q_max_star*((-upperLimit - lowerLimit)/(upperLimit - lowerLimit));
