...
import SI = ClaRa.Basics.Units;
  ...
  extends TransiEnt.Storage.Heat.PackedBedStorage_L4.Basics.PackedBedGeometry.PackedBedGeometry_N_cv(
    final circumference=ones(N_cv)*2*(width + height),
    final A_cross_bed=ones(N_cv)*width*height,
    final z_in=0,
    final z_out=0,
    final z=zeros(N_cv),
    final Delta_z_in=zeros(N_cv),
    final max_height=ones(N_cv)*height);
  ...
  parameter SI.Length height=1 "Height of the component" annotation(Dialog(group="Essential Geometry Definition"));
...
<p>Block shaped packed bed geometry. </p>
