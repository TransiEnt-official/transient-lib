...
public
  Modelica.Blocks.Sources.RealExpression m_flow_target_set(y=m_flow_target) annotation (Placement(transformation(extent={{78,-27},{58,-7}})));
  ...
  Modelica.Blocks.Continuous.FirstOrder m_flow(
    ...
    y_start=m_flow_init) annotation (Placement(transformation(extent={{12,-27},{-8,-7}})));


  TransiEnt.Components.Boundaries.FluidFlow.FluidSink sink2(h=95*4186) annotation (Placement(transformation(extent={{-28,10},{-48,30}})));
  ...
  TransiEnt.Components.Boundaries.FluidFlow.FluidSource fluidSource annotation (Placement(transformation(extent={{-30,-30},{-50,-10}})));
  Modelica.Blocks.Sources.RealExpression realExpression1(y=T_return_internal*cf)
                                                             annotation (Placement(transformation(extent={{30,-81},{10,-61}})));
equation
  // _____________________________________________
  //
  ...
  connect(m_flow_lim_set.y, m_flow.u) annotation (Line(points={{21,-17},{14,-17}},          color={0,0,127}));
  connect(m_flow_lim_set.u, m_flow_target_set.y) annotation (Line(points={{44,-17},{57,-17}},                   color={0,0,127}));
  connect(realExpression2.y,sink2. p_in) annotation (Line(points={{-13,62},{-24,62},{-24,20},{-30,20}},     color={0,0,127}));
  connect(sink2.port_a, fluidPortIn) annotation (Line(points={{-48,20},{-98,20}}, color={0,0,0}));
  connect(realExpression1.y,fluidSource. h_in) annotation (Line(points={{9,-71},{-26,-71},{-26,-22},{-32,-22}},  color={0,0,127}));
  connect(fluidSource.port_a, fluidPortOut) annotation (Line(points={{-50,-20},{-98,-20}}, color={0,0,0}));
  connect(m_flow.y, fluidSource.m_flow_in) annotation (Line(points={{-9,-17},{-32,-17}}, color={0,0,127}));
