...
public
  Components.Sensors.RealGas.GCVSensor                vleGCVSensor(medium=medium,flowDefinition=flowDefinition) annotation (Placement(transformation(extent={{-24,0},{-4,20}})));
...
equation
  ...
  connect(pipe_isoth.gasPortIn, fluidPortIn) annotation (Line(
      ...
      color={255,255,0},
      thickness=1.5));
  connect(vleGCVSensor.GCV_InToOut, gCVController.GCV_is_sink) annotation (Line(points={{-3,14},{0,14},{0,44},{76,44},{76,4.6}}, color={0,0,127}));
  connect(vleGCVSensor.GCV_OutToIn, gCVController.GCV_is_source) annotation (Line(points={{-3,6},{2,6},{2,38},{72,38},{72,4.6}}, color={0,0,127}));
  connect(H_flow_const_.y,gCVController.H_flow_set);
