...
public
  ...
  parameter Real v_n=exponentialStaticConsumerModel.simCenter.v_n
    "Nominal voltage";
  ...
  Modelica.Blocks.Math.Product ReactivePower annotation (Placement(
        ...
        rotation=0,
        origin={-2,60})));
  Modelica.Blocks.Math.Add ActiveSum1 annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=-90,
        origin={46,-22})));
  Modelica.Blocks.Math.Add ReactiveSum1 annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=-90,
        origin={66,-22})));
...
equation
  ...
  connect(electricFrequency_L1_1.f, exponentialStaticConsumerModel.f)
    annotation (Line(points={{-59.6,0},{-53.6,0},{-53.6,54},{-42.24,54}}, color=
         {0,0,127}));

  connect(modelStatistics.powerCollector[EnergyResource.Consumer],
    collectElectricPower.powerCollector);

  connect(electricFrequency_L1_1.v, exponentialStaticConsumerModel.v)
    annotation (Line(points={{-60,-6},{-60,-6},{-50,-6},{-50,44},{-48,44},{-42.24,
          44},{-42.24,43.2}}, color={0,0,127}));
  ...
  connect(boundary.epp, epp) annotation (Line(
      ...
      color={0,127,0},
      thickness=0.5));
  connect(P_set.y, ActiveSum1.u2) annotation (Line(points={{33,-8},{42,-8},{42,-14.8},
          {42.4,-14.8}}, color={0,0,127}));
  connect(ActivePower.y, ActiveSum1.u1) annotation (Line(points={{4.6,24},{49.3,
          24},{49.3,-14.8},{49.6,-14.8}}, color={0,0,127}));
  connect(Q_set.y, ReactiveSum1.u1) annotation (Line(points={{73,-4},{70,-4},{70,
          -14.8},{69.6,-14.8}}, color={0,0,127}));
  connect(ReactiveSum1.u2, ReactivePower.y) annotation (Line(points={{62.4,-14.8},
          {62.4,52.6},{4.6,52.6},{4.6,60}}, color={0,0,127}));
  connect(ActiveSum1.y, boundary.P_el_set)
    annotation (Line(points={{46,-28.6},{46,-42}}, color={0,0,127}));
  connect(ReactiveSum1.y, boundary.Q_el_set) annotation (Line(points={{66,-28.6},
          {62,-28.6},{62,-42},{58,-42}}, color={0,0,127}));
  connect(ReactivePower.u2, exponentialStaticConsumerModel.Delta_Q_star)
    annotation (Line(points={{-9.2,56.4},{-15.6,56.4},{-15.6,54},{-21.78,54}},
        color={0,0,127}));
  connect(ActivePower.u1, exponentialStaticConsumerModel.Delta_P_star)
    annotation (Line(points={{-9.2,27.6},{-9.2,34.8},{-21.78,34.8},{-21.78,42.8}},
        color={0,0,127}));
  connect(ReactivePower.u1, Q_el_n.y) annotation (Line(points={{-9.2,63.6},{-12,
          63.6},{-12,80},{-15,80}}, color={0,0,127}));
  connect(P_el_n.y, ActivePower.u2) annotation (Line(points={{-15,16},{-12,16},{
          -12,20.4},{-9.2,20.4}}, color={0,0,127}));
  annotation (
